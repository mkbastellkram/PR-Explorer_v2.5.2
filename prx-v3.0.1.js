/* ============================================================
   PR Explorer · V3.0.1 Additive Patch
...
function enhanceDetail(){
  const dc=$('#detailContent'); if(!dc || dc.__prx301Enhanced) return;
  const text=dc.textContent || '';
  const pr = data().find(p=> text.includes(prName(p)) || text.includes(prNum(p)) || (window.S?.selected && normId(p.id)===normId(window.S.selected.id)));
  if(!pr) return;
...
function exportZip(){
  ...
  const blob=zipStore(files); downloadBlob('PRX-roadmap-export-v3.0.1.zip', blob);
}
...
function zipStore(files){
  ...
  for(const [name,content] of Object.entries(files)){
    ...
    const local=arr(u32(0x04034b50),u16(20),u16(0),u16(0),u16(tm),u16(dt),u32(crc),u32(dataB.length),u32(dataB.length),u16(nameB.length),u16(0));
    chunks.push(local,nameB,dataB);
    central.push(arr(u32(0x02014b50),u16(20),u16(20),u16(0),u16(0),u16(tm),u16(dt),u32(crc),u32(dataB.length),u32(dataB.length),u16(nameB.length),u16(0),u16(0),u16(0),u16(0),u32(0),u32(offset)),nameB);
