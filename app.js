...
const APP_CHANGELOG = [
  ...
  { version:'V3.2.21', date:'2026-06-05', title:'Routenintelligenz & POI-Kontext', changes:[
    ...
    'KML-Anfahrten als primäre Quelle für Anfahrtsstopps; driveRoute/Home-Linie bleiben als Fallback erhalten.',
    'Zentrales Typografie-System als CSS-Variablen eingeführt (--fs-*, --fw-*, --lh-*).',
    'POI-Karten und Detailblöcke optisch überarbeitet.',
    'POI_ROUTE_SETTINGS-Konstante für konfigurierbare Abstände.',
    'Service-Worker-Cache und Asset-Versionierung auf V3.2.21 aktualisiert.'
  ]},
  ...
  { version:'V3.2.8', date:'2026-06-04', title:'Detail-Icon tatsächlich aktiv', changes:[
    ...
    'Oberer Karte-Textbutton bleibt entfernt; Schließen-X bleibt frei.',
    'Keine Änderung an Karteninitialisierung, setTab(), Service Worker, Safe-Area oder Bottom-Dock.',
  ...
  { version:'V3.2.5', date:'2026-06-04', title:'Cache- und Startdiagnose', changes:[
    ...
    'Debug-Panel in Optionen ergänzt; bei schwerem Startfehler erscheint ein sichtbarer Diagnosehinweis.',
    'Keine Navigations-, Detail-, Safe-Area-, Bottom-Dock- oder Overlay-Änderungen.',
    'Basis bleibt der funktionierende V3.2.3/V3.2.1-Stand.'
  ]},
  { version:'V3.2.3', date:'2026-06-03', title:'Recovery zurück auf V3.2.1', changes:[
    ...
    'Karte, PR-Pins, Reisen, Optionen, Sheet-Struktur und externe Aktivitäten entsprechen wieder V3.2.1.',
    'Keine V3.2.2-Navigationsfunktionen enthalten.',
    'Weiterhin keine aktiven prx-v*-Zusatzdateien.'
  ]},
...
function renderDetail(){
  ...
  qs('#detailContent').innerHTML=`
    ${v326DetailNavHtml(r)}
    <div class="v327-detail-title-grid">
      <div class="v327-detail-title-text">
    ...
    ${prxWeatherContainerHtml()}${prxWebcamContainerHtml()}<div class="elev-stats"><div class="elev-stat"><b>${fmt(r.elevMin||r.low)} m</b><small>Tiefpunkt</small></div><div class="elev-stat"><b>${fmt(r.elevMax||r.high)} m</b><small>Hochpunkt</small></div><div class="elev-stat"><b>↑ ${fmt(r.elevUp)} m</b><small>Aufstieg</small></div><div class="elev-stat"><b>↓ ${fmt(r.elevDown)} m</b><small>Abstieg</small></div></div></div>`:`${prxWeatherContainerHtml()}${prxWebcamContainerHtml()}`}
    <div class="route-intel-section">${prxPOIContainerHtml()}</div>
...
function v325RunDiagnostics(){
  const d={version:APP_VERSION,time:new Date().toISOString(),url:location.href,
    serviceWorkerControlled:!!navigator.serviceWorker?.controller,
    leafletLoaded:typeof L!=='undefined',
    mapContainer:!!document.getElementById('map'),
    dataArrayName:Array.isArray(window.PR_DATA)?'PR_DATA':(Array.isArray(window.DATA)?'DATA':'none'),
    dataCount:Array.isArray(window.PR_DATA)?window.PR_DATA.length:(Array.isArray(window.DATA)?window.DATA.length:0),
    appDataCount:Array.isArray(window.DATA)?window.DATA.length:0,
    mapObject:typeof map!=='undefined'&&!!map,
    mapHasSize:false,tilePanes:0,markerLikeLayers:0,errors:[]};
  try{
    if(d.mapObject){
      const s=map.getSize(); d.mapHasSize=!!(s&&s.x>20&&s.y>20);
      d.tilePanes=document.querySelectorAll('.leaflet-tile-pane img,.leaflet-tile-pane .leaflet-tile').length;
      let c=0; map.eachLayer(layer=>{try{if(layer instanceof L.Marker||layer instanceof L.CircleMarker||layer instanceof L.Polyline)c++;}catch(_){}}); d.markerLikeLayers=c;
    }
  }catch(e){d.errors.push('Map-Check: '+(e?.message||e));}
  if(!d.leafletLoaded)d.errors.push('Leaflet ist nicht geladen.');
  if(!d.mapContainer)d.errors.push('Map-Container #map fehlt.');
  if(d.dataCount<=0&&d.appDataCount<=0)d.errors.push('Keine PR-Daten gefunden.');
  if(d.mapObject&&!d.mapHasSize)d.errors.push('Leaflet-Map hat keine nutzbare Größe.');
  if(d.mapObject&&d.markerLikeLayers<=0)d.errors.push('Keine Marker/Tracks als Leaflet-Layer erkannt.');
  window.__PRX_LAST_DIAG=d; localStorage.setItem('prx_last_start_diagnose',JSON.stringify(d,null,2)); return d;
}
function v325DiagnosticsText(d){
  return [`PR Explorer · Startdiagnose ${d.version}`,`Zeit: ${new Date(d.time).toLocaleString('de-DE')}`,`Status: ${d.errors.length?'FEHLER / PRÜFEN':'OK'}`,'',
  `Service Worker kontrolliert: ${d.serviceWorkerControlled?'ja':'nein / erster Start'}`,`Leaflet geladen: ${d.leafletLoaded?'ja':'nein'}`,
  `Map-Container vorhanden: ${d.mapContainer?'ja':'nein'}`,`Datenquelle: ${d.dataArrayName}`,`PR-Datensätze: ${d.dataCount||d.appDataCount}`,
  `Map-Objekt: ${d.mapObject?'ja':'nein'}`,`Map-Größe nutzbar: ${d.mapHasSize?'ja':'nein'}`,`Tile-Elemente: ${d.tilePanes}`,`Marker/Track-Layer: ${d.markerLikeLayers}`,'','Fehler:',
  ...(d.errors.length?d.errors.map(e=>'- '+e):['- keine']),'',`URL: ${d.url}`].join('\n');
}
function v325DiagnosticsHtml(){
  const d=v325RunDiagnostics(), state=d.errors.length?'warn':'ok';
  return `<div class="p-section">Startdiagnose</div><div class="sg-box v325-diag ${state}">
    <div class="v325-diag-head"><b>${d.errors.length?'Prüfen':'OK'}</b><span>${APP_VERSION}</span></div>
    <div class="v325-grid">
      <span>Leaflet</span><b>${d.leafletLoaded?'ja':'nein'}</b><span>Daten</span><b>${d.dataCount||d.appDataCount}</b>
      <span>Map</span><b>${d.mapObject?'ja':'nein'}</b><span>Map-Größe</span><b>${d.mapHasSize?'ja':'nein'}</b>
      <span>Tiles</span><b>${d.tilePanes}</b><span>Layer</span><b>${d.markerLikeLayers}</b><span>SW</span><b>${d.serviceWorkerControlled?'aktiv':'nicht aktiv'}</b>
    </div>${d.errors.length?`<div class="v325-errors">${d.errors.map(e=>`<p>${htmlEsc(e)}</p>`).join('')}</div>`:`<p class="v325-ok">Karte/Daten wirken initialisiert.</p>`}
    <div class="v325-actions"><button class="mini-btn" onclick="v325CopyDiagnostics()">Diagnose kopieren</button><button class="mini-btn" onclick="v325HardReload()">Hard Reload</button></div>
  </div>`;
}
function v325CopyDiagnostics(){
  const txt=v325DiagnosticsText(v325RunDiagnostics());
  navigator.clipboard?.writeText(txt).then(()=>toast('Diagnose kopiert')).catch(()=>{const ta=document.createElement('textarea');ta.value=txt;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();toast('Diagnose kopiert');});
}
function v325HardReload(){ location.href=location.pathname+'?v='+Date.now(); }
function v325ShowStartupWarningIfNeeded(){
  setTimeout(()=>{const d=v325RunDiagnostics(); const severe=!d.leafletLoaded||!d.mapContainer||(d.dataCount<=0&&d.appDataCount<=0)||!d.mapObject;
    if(!severe)return; let box=document.getElementById('v325StartupWarn'); if(!box){box=document.createElement('div');box.id='v325StartupWarn';
      box.innerHTML=`<b>Startproblem erkannt</b><p>Karte/Daten konnten nicht sauber initialisiert werden.</p><button onclick="setTab('options')">Diagnose öffnen</button><button onclick="this.parentElement.remove()">Schließen</button>`;
      document.body.appendChild(box);}},1800);
}
...
function saveTestResults(){ const v=JSON.stringify(_testResults); localStorage.setItem('prTestResults',v); localStorage.setItem('prTestResultsPersistent',v); _updateTestBadge();v325ShowStartupWarningIfNeeded(); }
...
const POI_ROUTE_SETTINGS = {
  driveMaxDistanceKm: 1.5,   // Anfahrt: POIs bis 1.5 km seitlich
  hikeMaxDistanceKm:  0.8,   // Wanderroute: POIs bis 800 m
  accessMaxDistanceKm:1.5,   // Start/Ziel/Parkplatz: bis 1.5 km Radius
  maxDriveStops: 6,
  maxHikePois:   6,
  maxAccessPois: 4
};
...
function prxPOIContainerHtml(){
  return `<div id="poiDriveContainer" class="poi-container"></div>
          <div id="poiHikeContainer"  class="poi-container"></div>
          <div id="poiAccessContainer" class="poi-container"></div>`;
}
...
// KML-Anfahrtsdateien: echte Dateinamen aus Google Maps Export
const DRIVE_KML_MAP = {
  'PR 1':   'kml/Route von Pestana Promenade nach PR_1.kml',
  'PR 1.1': 'kml/Route von Pestana Promenade nach PR_1.1.kml',
  'PR 1.2': 'kml/Route von Pestana Promenade nach PR 1.2.kml',
  'PR 1.3': 'kml/Route von Pestana Promenade nach PR 1.3.kml',
  'PR 2':   'kml/Route von Pestana Promenade nach PR 2.kml',
  'PR 3':   'kml/Route von Pestana Promenade nach PR 3.kml',
  'PR 3.1': 'kml/Route von Pestana Promenade nach PR 3.1.kml',
  'PR 4':   'kml/Route von Pestana Promenade nach PR 4.kml',
  'PR 5':   'kml/Route von Pestana Promenade nach PR 5.kml',
  'PR 6':   'kml/Route von Pestana Promenade nach PR 6.kml',
  'PR 6.1': 'kml/Route von Pestana Promenade nach PR 6.1.kml',
  'PR 6.2': 'kml/Route von Pestana Promenade nach PR 6.2.kml',
  'PR 6.3': 'kml/Route von Pestana Promenade nach PR 6.3.kml',
  'PR 6.4': 'kml/Route von Pestana Promenade nach PR 6.4.kml',
  'PR 6.5': 'kml/Route von Pestana Promenade nach PR 6.5.kml',
  'PR 6.6': 'kml/Route von Pestana Promenade nach PR 6.6.kml',
  'PR 6.8': 'kml/Route von Pestana Promenade nach PR 6.8.kml',
  'PR 7':   'kml/Route von Pestana Promenade nach PR 7.kml',
  'PR 8':   'kml/Route von Pestana Promenade nach PR 8.kml',
  'PR 9':   'kml/Route von Pestana Promenade nach PR 9.kml',
  'PR 9.1': 'kml/Route von Pestana Promenade nach PR 9.1.kml',
  'PR 10':  'kml/Route von Pestana Promenade nach PR 10.kml',
  'PR 11':  'kml/Route von Pestana Promenade nach PR 11.kml',
  'PR 12':  'kml/Route von Pestana Promenade nach PR 12.kml',
  'PR 13':  'kml/Route von Pestana Promenade nach PR 13.kml',
  'PR 13.1':'kml/Route von Pestana Promenade nach PR 13.1.kml',
  'PR 14':  'kml/Route von Pestana Promenade nach PR 14.kml',
  'PR 15':  'kml/Route von Pestana Promenade nach PR 15.kml',
  'PR 16':  'kml/Route von Pestana Promenade nach PR 16.kml',
  'PR 17':  'kml/Route von Pestana Promenade nach PR17.kml',
  'PR 18':  'kml/Route von Pestana Promenade nach PR18.kml',
  'PR 19':  'kml/Route von Pestana Promenade nach PR19.kml',
  'PR 20':  'kml/Route von Pestana Promenade nach PR20.kml',
  'PR 21':  'kml/Route von Pestana Promenade nach PR21.kml',
  'PR 22':  'kml/Route von Pestana Promenade nach PR22.kml',
  'PR 27':  'kml/Route von Pestana Promenade nach PR27.kml',
  'PR 28':  'kml/Route von Pestana Promenade nach PR28.kml',
};
function prxDriveKmlPath(trail){
  if(!trail?.id) return null;
  return DRIVE_KML_MAP[trail.id] || null;
}
async function parseKMLtoPoints(kmlPath){
  if(!kmlPath) return [];
  const cacheKey = `kml_pts_${kmlPath}`;
  try{
  ...
  try{
    ...
    // Nur LineString-Placemarks verwenden — Point-Placemarks (Start/Ziel-Pins) überspringen
    const lineStrings = Array.from(xml.querySelectorAll('Placemark')).filter(pm =>
      pm.querySelector('LineString')
    );
    const points = [];
    const MAX_JUMP_DEG = 0.05; // >0.05° ≈ ~5 km = Datenfehler/Luftlinie
    lineStrings.forEach(pm => {
      const coordNode = pm.querySelector('LineString coordinates');
      if(!coordNode) return;
      let prev = null;
      String(coordNode.textContent || '').trim().split(/\s+/)
        .filter(s => s.includes(','))
        .forEach(s => {
          ...
          const pt = {lat, lng};
          if(prev){
            const d = Math.sqrt((lat-prev.lat)**2 + (lng-prev.lng)**2);
            if(d > MAX_JUMP_DEG){ prev = pt; return; } // Luftlinie überspringen
          }
          points.push(pt);
          prev = pt;
        });
    ...
    console.warn('[KML] Nicht verfügbar:', kmlPath, err?.message||err);
    return [];
  }
}
...
function poiSectionHtml(icon, title, sub, items){
  if(!items.length) return `<div class="poi-section poi-section-empty">
    <div class="poi-section-header"><span class="poi-section-title">${icon} ${htmlEsc(title)}</span></div>
    <div class="poi-empty">Keine passenden Stopps entlang dieser Route gefunden.</div>
  </div>`;
  return `<div class="poi-section">
    <div class="poi-section-header">
      <span class="poi-section-title">${icon} ${htmlEsc(title)}</span>
      <span class="poi-section-sub">${htmlEsc(sub)}</span>
    </div>
    <div class="poi-list">${items.join('')}</div>
  </div>`;
}

/* Block 1 — Stopps entlang der Anfahrt (driveRoute → KML → Home-Linie-Fallback) */
async function renderDriveStops(trail){
  const container = document.getElementById('poiDriveContainer');
  if(!container || !trail) return;
  container.innerHTML = '<div class="poi-loading">Anfahrtsstopps werden gesucht …</div>';

  let pathPoints = [];

  // Priorität 1: echte KML-Anfahrtsdatei aus kml/ Ordner
  const kmlPath = prxDriveKmlPath(trail);
  if(kmlPath) pathPoints = await parseKMLtoPoints(kmlPath);

  // Priorität 2: driveRoute aus pr-data.js als schneller Fallback
  if(!pathPoints.length && Array.isArray(trail.driveRoute) && trail.driveRoute.length > 2){
    pathPoints = trail.driveRoute.map(p => ({lat:p[0], lng:p[1]}));
  }

  // Priorität 3: Gerade Linie Hotel → PR-Start als Korridor
  if(!pathPoints.length){
    const home = cfg.home;
    const start = prxTrailStartCoords(trail);
    if(home?.lat && home?.lon && start){
      // Zwischenpunkte entlang der Linie erzeugen (alle 500m)
      const steps = 20;
      for(let i=0; i<=steps; i++){
        const t = i/steps;
        pathPoints.push({
          lat: home.lat + (start.lat - home.lat) * t,
          lng: home.lon + (start.lng - home.lon) * t
        });
      }
    }
  }
  ...
  let pois = [];
  if(pathPoints.length){
    pois = getPOIsAlongPath(pathPoints, POI_ROUTE_SETTINGS.driveMaxDistanceKm)
      .slice(0, POI_ROUTE_SETTINGS.maxDriveStops);
  }

  trail._drivePoiIds = new Set(pois.map(p=>p.id));

  if(!pois.length){
    container.innerHTML = `<div class="poi-section poi-section-empty">
      <div class="poi-section-header"><span class="poi-section-title">🚗 Stopps auf der Anfahrt</span></div>
      <div class="poi-empty">Keine Sehenswürdigkeiten nahe der Anfahrtsroute gefunden.</div>
    </div>`;
    return;
  }
...
/* Block 2 — Sehenswürdigkeiten nahe der Wanderroute (track) */
async function renderHikePOIs(trail){
  const container = document.getElementById('poiHikeContainer');
  if(!container || !trail) return;
  container.innerHTML = '<div class="poi-loading">Sehenswürdigkeiten am Wanderweg …</div>';

  const driveIds = trail._drivePoiIds || new Set();
  let pathPoints = [];
  ...
  let pois = [];
  if(pathPoints.length){
    pois = getPOIsAlongPath(pathPoints, POI_ROUTE_SETTINGS.hikeMaxDistanceKm)
      .filter(p => !driveIds.has(p.id))   // Duplikat-Filter
      .slice(0, POI_ROUTE_SETTINGS.maxHikePois);
  }

  trail._hikePoiIds = new Set(pois.map(p=>p.id));

  if(!pois.length){
    container.innerHTML = `<div class="poi-section poi-section-empty">
      <div class="poi-section-header"><span class="poi-section-title">🏛 Nahe der Wanderroute</span></div>
      <div class="poi-empty">Keine Sehenswürdigkeiten nahe dem Wanderweg gefunden.</div>
    </div>`;
    return;
  }
...
/* Block 3 — POIs nahe Start / Ziel / Parkplatz */
async function renderAccessPOIs(trail){
  const container = document.getElementById('poiAccessContainer');
  if(!container || !trail) return;
  container.innerHTML = '<div class="poi-loading">Zugangspunkte werden geprüft …</div>';

  const usedIds = new Set([...(trail._drivePoiIds||[]),...(trail._hikePoiIds||[])]);
  ...
  const dist = (p) => typeof haversineKm === 'function'
    ? haversineKm(c.lat, c.lng, p.lat, p.lng)
    : webcamHaversineKm(c.lat, c.lng, p.lat, p.lng);

  const pois = poisAll
    .map(p => ({...p, distanzKm: dist(p)}))
    .filter(p => p.distanzKm <= POI_ROUTE_SETTINGS.accessMaxDistanceKm && !usedIds.has(p.id))
    .sort((a,b) => a.distanzKm - b.distanzKm)
    .slice(0, POI_ROUTE_SETTINGS.maxAccessPois);

  if(!pois.length){
    container.innerHTML = `<div class="poi-section poi-section-empty">
      <div class="poi-section-header"><span class="poi-section-title">📍 Nahe Start &amp; Ziel</span></div>
      <div class="poi-empty">Keine weiteren Sehenswürdigkeiten nahe dem Startpunkt.</div>
    </div>`;
    return;
  }
