/* PR Explorer · V3.1.2 Clean Layer Reset Module */
(function(){
'use strict';

const PRX_VERSION='V3.1.2';
...
/* PR Explorer · V3.1.2 HUD Scope Runtime
...
/* PR Explorer · V3.1.2 Sheet and Control Normalizer */
(function(){
  'use strict';
  ...
  function closeElement(el){
    ...
    if(el.closest && el.closest('#prx310-overlay')){
      const overlay = qs('#prx310-overlay');
    ...
    // Fallback: hide instead of deleting legacy panels.
    el.classList.add('hidden');
    el.setAttribute('aria-hidden','true');
  }

  function addHandle(el){
    if(!el || el.__prx312Handled) return;
    if(el.id === 'bottomNav') return;
    if(el.id === 'prx310-tools') return;
    if(el.id === 'prx310ZoomSlider') return;

    // Do not add to tiny cards or nested controls.
    const r = el.getBoundingClientRect();
    if(r.height && r.height < 90) return;

    const handle = document.createElement('div');
    handle.className = 'prx312-handle-hit';
    handle.setAttribute('role','button');
    handle.setAttribute('aria-label','Nach unten ziehen zum Schließen');
    handle.innerHTML = '<div class="prx312-handle"></div>';

    // For V3 sheets, prepend as first element.
    // For legacy panels, also prepend; if this looks wrong, it is still safer than overlaying.
    el.insertBefore(handle, el.firstChild);
    el.__prx312Handled = true;
    ...
    function onDown(e){
      ...
      currentY = 0;
      dragging = true;
      el.classList.add('prx312-dragging');
    ...
    function onMove(e){
      ...
      if(currentY > 4){
        e.preventDefault();
        const dy = Math.min(currentY, 140);
        el.style.transform = `translateY(${dy}px)`;
        el.style.opacity = String(Math.max(.72, 1 - dy/420));
        el.classList.toggle('prx312-can-close', dy > 58);
      }
    }
    function onUp(){
      if(!dragging) return;
      dragging = false;
      el.classList.remove('prx312-dragging','prx312-can-close');
      const shouldClose = currentY > 58;
      el.style.transform = '';
      el.style.opacity = '';
      startY = null;
      currentY = 0;
      if(shouldClose) closeElement(el);
    }
    ...
    // Tapping the handle should not close immediately. It only provides drag affordance.
  }

  function normalizeSheets(){
    const selectors = [
      '#prx310-overlay .prx310-sheet',
      '.panel:not(#bottomNav)',
      '.panel-detail',
      '.settings-panel',
      '.sheet:not(#prx310-tools)',
      '.filter-panel',
      '.drawer'
    ];
    selectors.forEach(sel => qsa(sel).forEach(el => {
      if(isVisible(el)) addHandle(el);
    }));
  }

  function normalizeSwitches(){
    // Ensure all V3 checkbox switches are visible and have a useful label state.
    qsa('.prx310-switch input[type="checkbox"], #prx310Body input[type="checkbox"]').forEach(inp => {
      inp.classList.add('prx312-switch-native');
      inp.style.pointerEvents = 'auto';
      if(!inp.id){
        inp.id = 'prx312-switch-' + Math.random().toString(36).slice(2,8);
      }
      const label = inp.closest('label');
      if(label){
        label.classList.add('prx312-switch-row');
        label.style.pointerEvents = 'auto';
      }
    });
  }

  function normalizeCloseTargets(){
    qsa('.prx310-x,.detail-close,.sheet-close,.close,.close-btn,button[aria-label*="Schließen"],button[title*="Schließen"]').forEach(btn => {
  ...
  function normalize(){
    normalizeSheets();
    normalizeSwitches();
    normalizeCloseTargets();
    if(typeof window.PRX311_applyHudScope === 'function'){
      try { window.PRX311_applyHudScope(); } catch(e){}
    }
  }
  ...
  function boot(){
    ...
    document.addEventListener('click', () => setTimeout(normalize, 60), true);
    document.addEventListener('touchend', () => setTimeout(normalize, 60), true);
  ...
  window.PRX312_normalizeSheets = normalize;
})();
