/* ============================================================
   PR Explorer · V3.0.4 Additive Patch
...
function exportZip(){
  ...
  const blob=zipStore(files); downloadBlob('PRX-roadmap-export-v3.0.4.zip', blob);
}
...
/* ============================================================
   PR Explorer · V3.0.4 Runtime Hotfixes
   ============================================================ */
(function(){
  'use strict';
  const KEY_STATE='prx301_state';
...
/* ============================================================
   PR Explorer · V3.0.4 Final Runtime Corrections
   ============================================================ */
(function(){
  'use strict';
...
/* ============================================================
   PR Explorer · V3.0.4 iOS Safe-Area Runtime Fix
   ============================================================ */
(function(){
  'use strict';

  function setIOSViewportVars(){
    const vv = window.visualViewport;
    const h = Math.round((vv && vv.height) ? vv.height : window.innerHeight);
    document.documentElement.style.setProperty('--prx-visual-height', h + 'px');
    document.documentElement.classList.add('prx304-safearea');
  }

  function applySafeAreaClasses(){
    const bnav = document.querySelector('#bottomNav,.bnav');
    if (bnav) bnav.classList.add('prx-bottom-safe');
    document.querySelectorAll('.panel,.panel-detail,.test-panel,.settings-panel,.sheet,.prx301-sheet').forEach(el => {
      el.classList.add('prx-panel-safe');
    });
  }
  ...
  function bootSafeArea(){
    ...
    window.addEventListener('resize', setIOSViewportVars, { passive:true });
    window.addEventListener('orientationchange', () => setTimeout(setIOSViewportVars, 250), { passive:true });
    if (window.visualViewport) {
      window.visualViewport.addEventListener('resize', setIOSViewportVars, { passive:true });
      window.visualViewport.addEventListener('scroll', setIOSViewportVars, { passive:true });
    }
    const mo = new MutationObserver(() => applySafeAreaClasses());
    mo.observe(document.body, { childList:true, subtree:true });
  }

  if(document.readyState === 'loading') document.addEventListener('DOMContentLoaded', bootSafeArea);
  else bootSafeArea();
})();
